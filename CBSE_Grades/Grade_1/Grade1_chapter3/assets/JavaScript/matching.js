...
function init() {
  ...
  rightData.forEach((item) => {
    ...
    const imgSrc = leftData.find((l) => l.match === item.match).img;

    div.innerHTML = `
                    <img src="${imgSrc}" class="left-img">
                    <span>${item.text}</span>
                `;
...
function handleMatch(leftEl, rightEl) {
  ...
  const img = rightEl.querySelector("img").cloneNode(true);
  const box = leftEl.querySelector(".answer-box");
  box.appendChild(img);
