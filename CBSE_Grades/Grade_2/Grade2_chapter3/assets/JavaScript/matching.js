...
const images = {
  "Help filter blood and remove waste from the body":
    "../assets/images/kidney_function.png",
...
function attempt(el, p) {
  ...
  if (armed.p.r === p.r) {
    ...
    leftBadge.className = "match-number";
    leftBadge.textContent = matchNumber;

    const rightBadge = document.createElement("div");
    rightBadge.className = "match-number";
    rightBadge.textContent = matchNumber;

    armed.el.appendChild(leftBadge);
